...
export default function Details() {
  ...
  // لما المنتجات تتغير او id يتغير → دور على المنتج المناسب
  useEffect(() => {
    if (products.length > 0) {
      const foundProduct = products.find((p) => p.id == id); // لو id رقم
        ...
        <div className="flex flex-col lg:flex-row gap-6 w-full">
          ...
          <div className="propdetail border shadow-md p-4 flex flex-col gap-3 w-full lg:w-1/2">
            ...
            <div className="flex flex-wrap gap-2 text-gray-600 text-sm mt-2">
              <span className="border px-3 py-1 rounded-md hover:bg-teal-400">{currentProduct.category}</span>
              <span className="border px-3 py-1 rounded-md hover:bg-teal-400">{currentProduct.brand}</span>
              <span className="border px-3 py-1 rounded-md hover:bg-teal-400">{currentProduct.availabilityStatus}</span>
              <span className="border px-3 py-1 rounded-md hover:bg-teal-400">{currentProduct.stock}</span>
              <span className="border px-3 py-1 rounded-md hover:bg-teal-400">{currentProduct.rating} ★</span>
              <span className="border px-3 py-1 rounded-md hover:bg-teal-400">{currentProduct.discountPercentage}% off</span>
            </div>
            ...
            {/* add to cart */}
            <button className="bg-teal-400 py-1.5 !rounded-md hover:bg-teal-500 transition w-full lg:w-auto mt-2">
              Add to Cart
            </button>
